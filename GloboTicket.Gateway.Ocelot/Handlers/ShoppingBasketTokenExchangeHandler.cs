...
{
    public class ShoppingBasketTokenExchangeHandler : DelegatingHandler
    {
        ...
        // to run in parallel without blocking each other.  Refer to EventCatalogTokenExchangeHandler
        // (1, 1) = only one caller is allowed inside at a time - this is your gatekeeper (1 = gate is open)
        private static readonly SemaphoreSlim _tokenSemaphore = new(1, 1);
        private readonly ILogger<ShoppingBasketTokenExchangeHandler> _logger;
        private readonly string _clientId;
        private readonly string _clientSecret;
        private readonly string _shoppingBasketScope;

        public ShoppingBasketTokenExchangeHandler(IHttpClientFactory httpClientFactory,
            IClientAccessTokenCache clientAccessTokenCache, ILogger<ShoppingBasketTokenExchangeHandler> logger, IConfiguration configuration)
        {
            ...
            _clientId = tokenExchangeSection["ClientId"] ?? throw new InvalidOperationException($"{nameof(ShoppingBasketTokenExchangeHandler)}: TokenExchange ClientId not configured.");
            _clientSecret = tokenExchangeSection["ClientSecret"] ?? throw new InvalidOperationException($"{nameof(ShoppingBasketTokenExchangeHandler)}: TokenExchange ClientSecret not configured.");
            _shoppingBasketScope = tokenExchangeSection.GetSection("Scopes")["ShoppingBasket"] ?? throw new InvalidOperationException($"{nameof(ShoppingBasketTokenExchangeHandler)}: ShoppingBasket scope not configured.");

            _logger.LogInformation("Created {Handler} instance. HashCode={HashCode}", nameof(ShoppingBasketTokenExchangeHandler), GetHashCode());
        }
        ...
        {           
            var cacheKey = $"gatewaytodownstreamtokenexchangeclient:shoppingbasket:{scope}";
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            var scope = _shoppingBasketScope; //"openid profile shoppingbasket.fullaccess";
