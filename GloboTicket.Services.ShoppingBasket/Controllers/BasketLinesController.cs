...
namespace GloboTicket.Services.ShoppingBasket.Controllers
{
    ...
    public class BasketLinesController : ControllerBase
    {
        ...
        private readonly ILogger<BasketLinesController> logger;
        ...
        public BasketLinesController(IBasketRepository basketRepository,
            ...
            ILogger<BasketLinesController> logger)
        {
            ...
            Guid basketLineId)
        {
            ...
            var basketLine = await basketLinesRepository.GetBasketLineById(basketId, basketLineId);
            if (basketLine == null)
            {
                return NotFound();
            }
        ...
        {
            logger.LogInformation("Incoming basketId from route: {BasketId}", basketId);
            logger.LogInformation("BasketLineForCreation EventId: {EventId}", basketLineForCreation.EventId);

            var basket = await basketRepository.GetBasketById(basketId);

            if (basket == null)
            {
                logger.LogWarning("Basket {BasketId} not found.", basketId);
                return NotFound();
            }
            ...
            logger.LogInformation("Mapped BasketLineEntity BEFORE repo call: BasketId={BasketId}, BasketLineId={BasketLineId}",
                basketLineEntity.BasketId, basketLineEntity.BasketLineId);
            ...
            logger.LogInformation("ProcessedBasketLine AFTER repo call: BasketId={BasketId}, BasketLineId={BasketLineId}",
                processedBasketLine.BasketId, processedBasketLine.BasketLineId);

            var basketLineToReturn = mapper.Map<BasketLine>(processedBasketLine);
            logger.LogInformation("BasketLineToReturn AFTER AutoMapper: BasketId={BasketId}, BasketLineId={BasketLineId}",
                basketLineToReturn.BasketId, basketLineToReturn.BasketLineId);
            ...
            // logger.LogInformation("CreatedAtRoute will return: basketId={BasketId}, basketLineId={BasketLineId}",
            //     basketLineEntity.BasketId, basketLineEntity.BasketLineId);
            ...
            return CreatedAtRoute(
                "GetBasketLine",
                new { basketId = processedBasketLine.BasketId, basketLineId = processedBasketLine.BasketLineId },
                basketLineToReturn);
        }

        [HttpPut("{basketLineId}")]
        public async Task<ActionResult<BasketLine>> Put(Guid basketId, Guid basketLineId,
            [FromBody] BasketLineForUpdate basketLineForUpdate)
        {
            ...
            var basketLineEntity = await basketLinesRepository.GetBasketLineById(basketId, basketLineId);
            ...
            await basketLinesRepository.UpdateBasketLine(basketLineEntity);
            await basketLinesRepository.SaveChanges();
        ...
        public async Task<IActionResult> Delete(Guid basketId, Guid basketLineId)
        {
            ...
            await basketLinesRepository.RemoveBasketLine(basketLineEntity);
            await basketLinesRepository.SaveChanges();
