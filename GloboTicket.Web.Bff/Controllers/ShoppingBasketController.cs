...
namespace GloboTicket.Web.Controllers
{
    public class ShoppingBasketController : Controller
    {
        ...
        private async Task<BasketViewModel> CreateBasketViewModel()
        {
            ...
            BasketDto basket = await basketService.GetBasket(basketId);
        ...
        public async Task<IActionResult> AddLine(BasketLineForCreationDto basketLine)
        {
            var basketId = Request.Cookies.GetCurrentBasketId(settings);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> UpdateLine(BasketLineForUpdateDto basketLineUpdate)
        {
            var basketId = Request.Cookies.GetCurrentBasketId(settings);
        ...
        public async Task<IActionResult> Checkout(BasketCheckoutViewModel basketCheckoutViewModel)
        {
            try
            {
                ...
                {
                    var basketForCheckout = new BasketForCheckoutDto
                    {
        ...
        public async Task<IActionResult> ApplyDiscountCode(string code)
        {
            ...
            var basketId = Request.Cookies.GetCurrentBasketId(settings);
            await basketService.ApplyCouponToBasket(basketId, new CouponForUpdateDto() { CouponId = coupon.CouponId });
