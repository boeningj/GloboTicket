        ...
        public async Task<IActionResult> Checkout(BasketCheckoutViewModel basketCheckoutViewModel)
        {
            try
            {
                ...
                if (!ModelState.IsValid)
                {
                    ...
                    ViewBag.ErrorMessage = "Invalid model: " + string.Join("; ", errors);
                ...
                if (ModelState.IsValid)
                {
                    var basketForCheckout = new BasketForCheckout
                    {
                        ...
                        BasketId = basketId,
                        UserId = User.GetUserId() //settings.UserId
                    };

                    await basketService.Checkout(basketForCheckout.BasketId, basketForCheckout);
